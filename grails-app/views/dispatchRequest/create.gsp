...
<script>
    function successCallBack(json) {
        if (json.success) {
            window.location.href = "${createLink(controller: 'dispatchRequest',action: 'createNewDispatchRequest')}?employeeId=" + json.employeeId;
        }
    }
...
<lay:widget title="${title}">
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <btn:listButton
                    onClick="window.location.href='${createLink(controller: 'dispatchRequest', action: 'list')}'"/>
        </btn:buttonGroup>
    </lay:widgetToolBar>
    <lay:widgetBody>
        <el:row>
            ...
            <msg:page/>
            <el:validatableResetForm name="dispatchRequestForm" callBackFunction="successCallBack"
                                     controller="dispatchRequest" action="selectEmployee">
