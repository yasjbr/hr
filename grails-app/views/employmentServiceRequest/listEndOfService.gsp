...
<head>
    ...
    <g:set var="entities" value="${message(code: 'endOfService.entities', default: 'endOfService List')}"/>
    <g:set var="entity" value="${message(code: 'endOfService.entity', default: 'endOfService')}"/>
    <g:set var="title" value="${message(code: 'default.list.label', args: [entities], default: 'endOfService List')}"/>
    <title>${title}</title>
</head>
...
<lay:collapseWidget id="employmentServiceRequestCollapseWidget" icon="icon-search"
    ...
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <btn:createButton
                    onClick="window.location.href='${createLink(controller: 'employmentServiceRequest', action: 'redirectEndOfService')}'"/>
        </btn:buttonGroup>
    </lay:widgetToolBar>
    <lay:widgetBody>
        <el:form action="#" name="employmentServiceRequestSearchForm">
            ...
            <g:render template="/employmentServiceRequest/search" model="[endOfServiceFlag:'YES']"/>
            <el:formButton functionName="search" onClick="_dataTables['employmentServiceRequestTable'].draw()"/>
            <el:formButton functionName="clear"
                           onClick="gui.formValidatable.resetForm('employmentServiceRequestSearchForm');_dataTables['employmentServiceRequestTable'].draw();"/>
        </el:form>
    </lay:widgetBody>
</lay:collapseWidget>
<el:dataTable id="employmentServiceRequestTable" searchFormName="employmentServiceRequestSearchForm"
              dataTableTitle="${title}"
              hasCheckbox="true" widthClass="col-sm-12" controller="employmentServiceRequest" spaceBefore="true"
              hasRow="true" action="filter" serviceName="employmentServiceRequest">
    ...
    <el:dataTableAction controller="employmentServiceRequest" action="goToList" showFunction="manageListLink"
                        ...
                        message="${message(code: 'endOfServiceList.entities', default: 'employmentServiceRequest')}"/>
