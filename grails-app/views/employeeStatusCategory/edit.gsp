...
<head>
    ...
    <g:set var="title" value="${message(code: 'default.edit.label',args:[entity], default: 'EmployeeStatusCategory List')}" />
    <title>${title}</title>
...
<lay:widget title="${title}">
    ...
    <lay:widgetBody>
        <el:row>
            <msg:page />
            <el:validatableForm name="employeeStatusCategoryForm" controller="employeeStatusCategory" action="update">
                <g:render template="/employeeStatusCategory/form" model="[employeeStatusCategory:employeeStatusCategory]"/>
                <el:hiddenField name="id" value="${employeeStatusCategory?.id}" />
