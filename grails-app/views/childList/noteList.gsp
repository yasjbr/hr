...
<g:set var="dataTableTitle" value="${message(code: 'default.list.label', args: [entities], default: 'childListEmployeeNote List')}"/>
...
<lay:collapseWidget id="childListEmployeeNoteCollapseWidget" icon="icon-search"
    ...
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <el:modalLink preventCloseOutSide="true" class="btn btn-sm btn-info2 width-135"
                          link="${createLink(controller: 'childList', action: 'noteCreate',id:id)}"
    ...
    <lay:widgetBody>
        <el:form action="#" name="childListEmployeeNoteSearchForm">
            <el:hiddenField name="childListEmployeeId" value="${id}"/>
            <g:render template="noteSearch" model="[:]"/>
            <el:formButton functionName="search" onClick="_dataTables['employeeNoteTable'].draw()"/>
            <el:formButton functionName="clear" onClick="gui.formValidatable.resetForm('childListEmployeeNoteSearchForm');_dataTables['employeeNoteTable'].draw();"/>
        </el:form>
    </lay:widgetBody>
</lay:collapseWidget>

<el:dataTable id="employeeNoteTable"
              searchFormName="childListEmployeeNoteSearchForm"
              ...
              hasRow="true"
              action="filter"
              serviceName="childListEmployeeNote">
<el:dataTableAction controller="childListEmployeeNote" action="delete" actionParams="encodedId"
class="red icon-trash" type="confirm-delete"
message="${message(code: 'default.delete.label', args: [entity], default: 'delete childListEmployeeNote')}"/>
</el:dataTable>
