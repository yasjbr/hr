...
<head>
    ...
    <g:set var="title" value="${message(code: 'default.edit.label',args:[entity], default: 'VacationListEmployee List')}" />
    <title>${title}</title>
...
<lay:widget title="${title}">
    ...
    <lay:widgetBody>
        <el:row>
            <msg:page />
            <el:validatableForm name="vacationListEmployeeForm" controller="vacationListEmployee" action="update">
                <g:render template="/vacationListEmployee/form" model="[vacationListEmployee:vacationListEmployee]"/>
                <el:hiddenField name="id" value="${vacationListEmployee?.id}" />
