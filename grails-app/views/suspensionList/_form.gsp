<el:formGroup>
    <el:textField name="name" size="8" class=" isRequired"
                  label="${message(code: 'suspensionList.name.label', default: 'name')}" value="${suspensionList?.name}"/>
</el:formGroup>
...
<el:formGroup>
    <el:select valueMessagePrefix="EnumReceivingParty" from="${ps.gov.epsilon.hr.enums.v1.EnumReceivingParty.values()}"
               ...
               value="${suspensionList?.receivingParty}"/>
</el:formGroup>
...
<el:formGroup>
    <el:textAreaDescription name="coverLetter" size="8" class=" "
                            ...
                            value="${suspensionList?.coverLetter}" />
