<!DOCTYPE html>
<html>
<head>
    <meta name="layout" content="mainGUI" />
    <g:set var="entity" value="${message(code: 'vacationConfiguration.entity', default: 'VacationConfiguration List')}" />
    <g:set var="title" value="${message(code: 'default.edit.label',args:[entity], default: 'VacationConfiguration List')}" />
    <title>${title}</title>
    <g:render template="script"/>
</head>
<body>
<lay:widget title="${title}">
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <btn:listButton onClick="window.location.href='${createLink(controller:'vacationConfiguration',action:'list')}'"/>
        </btn:buttonGroup>
    </lay:widgetToolBar>
    <lay:widgetBody>
        <el:row>
            <msg:page />
            <el:validatableForm name="vacationConfigurationForm" controller="vacationConfiguration" action="update">
                <g:render template="/vacationConfiguration/form" model="[vacationConfiguration:vacationConfiguration]"/>
                <el:hiddenField name="id" value="${vacationConfiguration?.id}" />
                <el:formButton isSubmit="true" withPreviousLink="true"  functionName="save"/>
                <el:formButton functionName="cancel" goToPreviousLink="true"/>
            </el:validatableForm>
        </el:row>
    </lay:widgetBody>
</lay:widget>
</body>
</html>