...
<head>
    ...
    <g:set var="entities" value="${message(code: 'recallToServiceList.entities', default: 'recallToServiceList List')}"/>
    <g:set var="entity" value="${message(code: 'recallToServiceList.entity', default: 'recallToServiceList')}"/>
    <g:set var="title"
           value="${message(code: 'default.list.label', args: [entities], default: 'recallToServiceList List')}"/>
    <title>${title}</title>
</head>
...
<lay:collapseWidget id="serviceListCollapseWidget" icon="icon-search"
    ...
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <btn:createButton
                    onClick="window.location.href='${createLink(controller: 'serviceList', action: 'createReturnToServiceList')}'"/>
        </btn:buttonGroup>
    </lay:widgetToolBar>
    <lay:widgetBody>
        <el:form action="#" name="serviceListSearchForm">
            <el:hiddenField name="isRecallToServiceType" value="true"/>
