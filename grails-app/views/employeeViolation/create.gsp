...
<head>
    ...
    <script>
        function successCallBack(json) {
            if (json.success) {
                window.location.href = "${createLink(controller: 'employeeViolation',action: 'createNewEmployeeViolation')}?employeeId=" + json.employeeId;
            }
        }
    </script>
</head>
...
<lay:widget title="${title}">
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <btn:listButton onClick="window.location.href='${createLink(controller:'employeeViolation',action:'list')}'"/>
        </btn:buttonGroup>
    </lay:widgetToolBar>
    <lay:widgetBody>
        <el:row>
            ...
            <el:validatableResetForm name="employeeViolationForm" callBackFunction="successCallBack"
                                     controller="employeeViolation" action="selectEmployee">
