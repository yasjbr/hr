<g:render template="/pcore/person/wrapper" model="[id:(personCallBackId?:'personId'),
                                             ...
                                             bean:personLanguageInfo?.person,
                                             isDisabled:isPersonDisabled]" />
<el:formGroup>
    <el:autocomplete optionKey="id" optionValue="name" size="8" class=" isRequired" controller="language" action="autocomplete" name="language.id" label="${message(code:'personLanguageInfo.language.label',default:'language')}" values="${[[personLanguageInfo?.language?.id,personLanguageInfo?.language?.descriptionInfo?.localName]]}" />
</el:formGroup>

<el:formGroup>
    <el:autocomplete optionKey="id" optionValue="name" size="8" class=" isRequired" controller="educationLevel" action="autocomplete" name="readingLevel.id" label="${message(code:'personLanguageInfo.readingLevel.label',default:'readingLevel')}" values="${[[personLanguageInfo?.readingLevel?.id,personLanguageInfo?.readingLevel?.descriptionInfo?.localName]]}" />
</el:formGroup>
<el:formGroup>
    <el:autocomplete optionKey="id" optionValue="name" size="8" class=" isRequired" controller="educationLevel" action="autocomplete" name="verbalLevel.id" label="${message(code:'personLanguageInfo.verbalLevel.label',default:'verbalLevel')}" values="${[[personLanguageInfo?.verbalLevel?.id,personLanguageInfo?.verbalLevel?.descriptionInfo?.localName]]}" />
</el:formGroup>
<el:formGroup>
