...
<head>
    ...
    <g:set var="title" value="${message(code: 'default.edit.label',args:[entity], default: 'VacationListEmployeeNote List')}" />
    <title>${title}</title>
...
<lay:widget title="${title}">
    ...
    <lay:widgetBody>
        <el:row>
            <msg:page />
            <el:validatableForm name="vacationListEmployeeNoteForm" controller="vacationListEmployeeNote" action="update">
                <g:render template="/vacationListEmployeeNote/form" model="[vacationListEmployeeNote:vacationListEmployeeNote]"/>
                <el:hiddenField name="id" value="${vacationListEmployeeNote?.id}" />
