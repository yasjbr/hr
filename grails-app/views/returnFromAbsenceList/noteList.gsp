...
<g:set var="dataTableTitle" value="${message(code: 'default.list.label', args: [entities], default: 'returnFromAbsenceListEmployeeNote List')}"/>
...
<lay:collapseWidget id="returnFromAbsenceListEmployeeNoteCollapseWidget" icon="icon-search"
    ...
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <el:modalLink preventCloseOutSide="true" class="btn btn-sm btn-info2 width-135"
                          link="${createLink(controller: 'returnFromAbsenceList', action: 'noteCreate',id:id)}"
    ...
    <lay:widgetBody>
        <el:form action="#" name="returnFromAbsenceListEmployeeNoteSearchForm">
            <el:hiddenField name="returnFromAbsenceListEmployeeId" value="${id}"/>
            <g:render template="noteSearch" model="[:]"/>
            <el:formButton functionName="search" onClick="_dataTables['employeeNoteTable'].draw()"/>
            <el:formButton functionName="clear" onClick="gui.formValidatable.resetForm('returnFromAbsenceListEmployeeNoteSearchForm');_dataTables['employeeNoteTable'].draw();"/>
        </el:form>
    </lay:widgetBody>
</lay:collapseWidget>

<el:dataTable id="employeeNoteTable"
              searchFormName="returnFromAbsenceListEmployeeNoteSearchForm"
              ...
              hasRow="true"
              action="filter"
              serviceName="returnFromAbsenceListEmployeeNote">
<el:dataTableAction controller="returnFromAbsenceListEmployeeNote" action="delete" actionParams="encodedId"
class="red icon-trash" type="confirm-delete"
message="${message(code: 'default.delete.label', args: [entity], default: 'delete returnFromAbsenceListEmployeeNote')}"/>
</el:dataTable>
