...
<g:set var="dataTableTitle" value="${message(code: 'default.list.label', args: [entities], default: 'petitionListEmployeeNote List')}"/>
...
<lay:collapseWidget id="petitionListEmployeeNoteCollapseWidget" icon="icon-search"
    ...
    <lay:widgetToolBar>
        <btn:buttonGroup>
            <el:modalLink preventCloseOutSide="true" class="btn btn-sm btn-info2 width-135"
                          link="${createLink(controller: 'petitionList', action: 'noteCreate',id:id)}"
    ...
    <lay:widgetBody>
        <el:form action="#" name="petitionListEmployeeNoteSearchForm">
            <el:hiddenField name="petitionListEmployeeId" value="${id}"/>
            <g:render template="noteSearch" model="[:]"/>
            <el:formButton functionName="search" onClick="_dataTables['employeeNoteTable'].draw()"/>
            <el:formButton functionName="clear" onClick="gui.formValidatable.resetForm('petitionListEmployeeNoteSearchForm');_dataTables['employeeNoteTable'].draw();"/>
        </el:form>
    </lay:widgetBody>
</lay:collapseWidget>

<el:dataTable id="employeeNoteTable"
              searchFormName="petitionListEmployeeNoteSearchForm"
              ...
              hasRow="true"
              action="filter"
              serviceName="petitionListEmployeeNote">
<el:dataTableAction controller="petitionListEmployeeNote" action="delete" actionParams="encodedId"
class="red icon-trash" type="confirm-delete"
message="${message(code: 'default.delete.label', args: [entity], default: 'delete petitionListEmployeeNote')}"/>
</el:dataTable>
