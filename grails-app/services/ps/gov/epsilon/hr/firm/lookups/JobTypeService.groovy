...
/**
 * <h1>Purpose</h1>
 * this service is aims to create job type for firm
 * <h1>Usage</h1>
 * -used for firm
 * <h1>Restriction</h1>
...
class JobTypeService {
    ...
    PagedResultList search(GrailsParameterMap params) {
        ...
        return JobType.createCriteria().list(max: max, offset: offset) {
            if (sSearch) {
                or {
 ...
 * @return JobType.
 */
    JobType save(GrailsParameterMap params) {
        JobType jobTypeInstance
        if (params.encodedId) {
            params.id = HashHelper.decode(params.encodedId)
        }
        if (params["id"]) {
            jobTypeInstance = JobType.get(params["id"])
            if (params.long("version")) {
                long version = params.long("version")
                if (jobTypeInstance.version > version) {
                    jobTypeInstance.errors.reject('default.optimistic.locking.failure', [messageSource.getMessage('jobType.label', null, 'jobType', LocaleContextHolder.getLocale())] as Object[], "Another user has updated this jobType while you were editing")
                    return jobTypeInstance
                }
            }
            if (!jobTypeInstance) {
                jobTypeInstance = new JobType()
                jobTypeInstance.errors.reject('default.not.found.message', [messageSource.getMessage('jobType.label', null, 'jobType', LocaleContextHolder.getLocale())] as Object[], "This jobType with ${params["id"]} not found")
                return jobTypeInstance
            }
        } else {
            jobTypeInstance = new JobType()
        ...
        try {
            ...
            jobTypeInstance.save();
        }
        catch (Exception ex) {
            transactionStatus.setRollbackOnly()
            jobTypeInstance.errors.reject('default.internal.server.error', [ex?.cause?.localizedMessage?.substring(0, ex?.cause?.localizedMessage?.indexOf("Detail:"))] as Object[], "")
        }
        return jobTypeInstance
    }
 ...
 */
    DeleteBean delete(DeleteBean deleteBean, boolean isEncrypted = false) {
        try {
            ...
            JobType instance = JobType.get(id)
 ...
 */
    @Transactional(readOnly = true)
    JobType getInstance(GrailsParameterMap params) {
        if (params.encodedId) {
            params.id = HashHelper.decode(params.encodedId)
        }
