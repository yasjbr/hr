...
/**
 * <h1>Purpose</h1>
 * -this service is aim to create external transfer list
 * <h1>Usage</h1>
 * -this service is used to create external transfer list
 * <h1>Restriction</h1>
 * -need a request & employee created before
 * @see MessageSource
 * @see FormatService
 * */
@Transactional
class ExternalTransferListService {
    ...
    public static currentStatusValue = { cService, ExternalTransferList rec, object, params ->
        return rec?.currentStatus?.correspondenceListStatus?.toString()
    }

    // to make name of list as link
    public static getListName = { formatService, ExternalTransferList dataRow, object, params ->
        if (dataRow) {
            return "<a href ='../externalTransferList/manageExternalTransferList?encodedId=${dataRow?.encodedId}'>${dataRow?.name?.toString()}</a>";
        }
        return ""
    }
    ...
    PagedResultList search(GrailsParameterMap params) {
        ...
        Set externalTransferListEmployeesIds = params.listString("externalTransferListEmployees.id")

        return ExternalTransferList.createCriteria().list(max: max, offset: offset) {
            if (sSearch) {
                or {
                    ilike("code", sSearch)
            ...
            and {
                ...
                if (externalTransferListEmployeesIds) {
                    externalTransferListEmployees {
                        inList("id", externalTransferListEmployeesIds)
                    }
                }
            }
            if (orderBy) {
    ...
    /**
     * custom search to find the number of requests in the externalTransfer  list in one select statement for performance issue
    ...
    PagedList customSearch(GrailsParameterMap params) {
        ...
        ZonedDateTime fromSendDate = PCPUtils.parseZonedDateTime(params['fromDateFrom'])
        ZonedDateTime toSendDate = PCPUtils.parseZonedDateTime(params['fromDateTo'])

        ZonedDateTime fromReceiveDate = PCPUtils.parseZonedDateTime(params['toDateFrom'])
        ZonedDateTime toReceiveDate = PCPUtils.parseZonedDateTime(params['toDateTo'])
        ...
        //the query to retrieve the list details, num of externalTransfers in the list, the send date, and the current list status
        String query = "FROM external_transfer_list al  LEFT JOIN " +
                "  (SELECT ale.external_transfer_list_id ,count(ale.id) no_of_employee" +
                "  FROM external_transfer_list_employee ale " +
                "  group by ale.external_transfer_list_id ) b" +
                "  on al.id= b.external_transfer_list_id , correspondence_list_status cls,correspondence_list cl" +
        ...
        List<ExternalTransferList> results = []
        ...
        queryResults.each { resultRow ->
            ...
            ExternalTransferList externalTransferList = new ExternalTransferList(
                    code: resultRow[1],
                    name: resultRow[2],
            ...
            externalTransferList.id = resultRow[0]
            ...
            externalTransferList.currentStatus = currentStatus
            results.add(externalTransferList)
        }
 ...
 */
    ExternalTransferList save(GrailsParameterMap params) {
        ExternalTransferList externalTransferListInstance
        ...
        if (params.id) {
            externalTransferListInstance = ExternalTransferList.get(params["id"])
            if (params.long("version")) {
                long version = params.long("version")
                if (externalTransferListInstance.version > version) {
                    externalTransferListInstance.errors.reject('default.optimistic.locking.failure', [messageSource.getMessage('externalTransferList.label', null, 'externalTransferList', LocaleContextHolder.getLocale())] as Object[], "Another user has updated this externalTransferList while you were editing")
                    return externalTransferListInstance
                }
            }
            if (!externalTransferListInstance) {
                externalTransferListInstance = new ExternalTransferList()
                externalTransferListInstance.errors.reject('default.not.found.message', [messageSource.getMessage('externalTransferList.label', null, 'externalTransferList', LocaleContextHolder.getLocale())] as Object[], "This externalTransferList with ${params.id} not found")
                return externalTransferListInstance
            }
        } else {
            externalTransferListInstance = new ExternalTransferList()
        ...
        try {
            externalTransferListInstance.properties = params;
            ...
            if(listStatus){
                correspondenceListStatus = new CorrespondenceListStatus(fromDate: ZonedDateTime.now(), toDate: PCPUtils.DEFAULT_ZONED_DATE_TIME,
                        correspondenceListStatus: listStatus, receivingParty: EnumReceivingParty.SARAYA, firm: externalTransferListInstance.firm)
                externalTransferListInstance.addToCorrespondenceListStatuses(correspondenceListStatus)
            }

            externalTransferListInstance.save(flush: true, failOnError: true);

            //save the current status:
            if (correspondenceListStatus?.id && externalTransferListInstance?.id) {
                externalTransferListInstance?.currentStatus = correspondenceListStatus
                externalTransferListInstance.save(flush: true, failOnError: true)
            }
        }
        catch (Exception ex) {
            transactionStatus.setRollbackOnly()
            externalTransferListInstance.errors.reject('default.internal.server.error', [ex?.cause?.localizedMessage?.substring(0, ex?.cause?.localizedMessage?.indexOf("Detail:"))] as Object[], "")
        }
        return externalTransferListInstance
    }
 ...
 */
    DeleteBean delete(DeleteBean deleteBean, boolean isEncrypted = false) {
        try {
            ...
            ExternalTransferList instance = ExternalTransferList.get(id)
            //to be able to delete an externalTransfer list when status is created
            if (instance?.currentStatus?.correspondenceListStatus in [EnumCorrespondenceListStatus.CREATED] && instance?.externalTransferListEmployees?.size() == 0) {
                ...
                deleteBean.responseMessage << messageSource.getMessage('externalTransferList.deleteMessage.label')
            }
        }
        catch (Exception ex) {
            deleteBean.status = false
 ...
 */
    @Transactional(readOnly = true)
    ExternalTransferList getInstance(GrailsParameterMap params) {
        if (params.encodedId) {
            params.id = HashHelper.decode(params.encodedId)
        }
     ...
     */
    @Transactional(readOnly = true)
    ExternalTransferList getInstanceWithRemotingValues(GrailsParameterMap params) {
        if (params.encodedId) {
            params.id = HashHelper.decode(params.encodedId)
        }
        //if id is not null then return values from search method
 ...
 */
    ...
    /**
     * to add externalTransferRequests to externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList addExternalTransferRequests(GrailsParameterMap params) {

        /**
         *  get list of ids of selected externalTransfer request
         */
        List checkedEncodedExternalTransferRequestIdList = params.listString("checked_requestIdsList")


        ExternalTransferList externalTransferList = ExternalTransferList?.load(params["id"])

        GrailsParameterMap externalTransferRequestMap
        ExternalTransferRequest externalTransferRequest = null
        List<ExternalTransferRequest> externalTransferRequestList
        ExternalTransferListEmployee externalTransferListEmployee

        if (checkedEncodedExternalTransferRequestIdList) {

            /**
             * get selected externalTransfer request
             */
            externalTransferRequestMap = new GrailsParameterMap(["ids[]": checkedEncodedExternalTransferRequestIdList], WebUtils.retrieveGrailsWebRequest().getCurrentRequest())
            externalTransferRequestList = externalTransferRequestService?.search(externalTransferRequestMap)

            /**
             * add all selected externalTransfer request to externalTransfer list
             */
            checkedEncodedExternalTransferRequestIdList?.each { String id ->

                /**
                 * change externalTransfer request status to ADD_TO_LIST
                 */
                externalTransferRequest = externalTransferRequestList?.find { it.id == id }
                if (externalTransferRequest) {
                    externalTransferRequest?.requestStatus = EnumRequestStatus.ADD_TO_LIST
                    externalTransferRequest?.validate()
                    externalTransferRequest?.save(flush:true, failOnError:true)

                    /**
                     * add externalTransfer request to externalTransfer list employee
                     */
                    externalTransferListEmployee = new ExternalTransferListEmployee()
                    externalTransferListEmployee?.externalTransferRequest = externalTransferRequest
                    externalTransferListEmployee?.recordStatus = EnumListRecordStatus.NEW
                    externalTransferListEmployee?.externalTransferList = externalTransferList
                    externalTransferListEmployee?.employee = externalTransferRequest?.employee
                    externalTransferListEmployee?.toOrganizationId = externalTransferRequest?.toOrganizationId
                    externalTransferListEmployee?.currentEmploymentRecord = externalTransferRequest?.currentEmploymentRecord
                    externalTransferListEmployee?.effectiveDate = PCPUtils.DEFAULT_ZONED_DATE_TIME

                    if (externalTransferRequest?.requestStatusNote) {
                        ExternalTransferListEmployeeNote note = new ExternalTransferListEmployeeNote(
                                noteDate: externalTransferRequest?.requestDate,
                                note: externalTransferRequest?.requestStatusNote,
                                orderNo: "",
                                externalTransferListEmployee: externalTransferListEmployee,
                        );
                        if (!note?.validate()) {
                            externalTransferList?.errors.addAllErrors(note?.errors)
                            throw new Exception("Error occurred while add note to the list employee record.")
                        } else {
                            //add note to join
                            externalTransferListEmployee.addToExternalTransferListEmployeeNotes(note);
                        }
                    }

                    /**
                     * add the externalTransfer list employee to externalTransfer list
                     */
                    externalTransferList?.addToExternalTransferListEmployees(externalTransferListEmployee)
                }
                externalTransferList?.save(flush:true, failOnError:true)
            }



            try {
                externalTransferList.validate()
                externalTransferList?.save(failOnError: true)
            } catch (Exception ex) {
                transactionStatus.setRollbackOnly()
                if (externalTransferList?.hasErrors()) {
                    externalTransferList.errors.reject('default.internal.server.error', [ex?.cause?.localizedMessage?.substring(0, ex?.cause?.localizedMessage?.indexOf("Detail:"))] as Object[], "")
                }
            }
        } else {
            externalTransferList.errors.reject('externalTransferList.error.not.selected.request.message')
            return externalTransferList
        }
        return externalTransferList
    }
    ...
    /**
     * send externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList sendData(GrailsParameterMap params) {

        //return externalTransferListId
        if (params.encodedId) {
            params.id = HashHelper.decode(params.encodedId)
        }
        if (params.id) {
            //return the corresponding list:
            ExternalTransferList externalTransferList = ExternalTransferList.get(params["id"])
            //return error if no request added to list
            if (externalTransferList?.externalTransferListEmployees?.size() == 0) {
                externalTransferList.errors.reject("list.sendList.error")
                return externalTransferList
            }
            if (externalTransferList) {
                try {
                    ...
                    correspondenceListStatus.correspondenceList = externalTransferList
                    ...
                    correspondenceListStatus.firm = externalTransferList?.firm
                    correspondenceListStatus.correspondenceListStatus = EnumCorrespondenceListStatus.SUBMITTED
                    if (correspondenceListStatus.save()) {
                        externalTransferList.currentStatus = correspondenceListStatus
                    }
                    //enter the manualIncomeNo when we send the marital status list
                    externalTransferList.manualOutgoingNo = params.manualOutgoingNo
                    //loop in all externalTransferListEmployee in marital status list to change the status of the request
                    externalTransferList?.externalTransferListEmployees.each { ExternalTransferListEmployee externalTransferListEmployee ->
                        externalTransferListEmployee?.externalTransferRequest?.requestStatus = EnumRequestStatus.SENT_BY_LIST
                        externalTransferListEmployee?.externalTransferRequest?.validate()
                    }

                    //save the externalTransfer list changes
                    externalTransferList.save(failOnError: true)
                } catch (Exception ex) {
                    ex.printStackTrace()
                    transactionStatus.setRollbackOnly()
                    if (externalTransferList?.errors?.allErrors?.size() == 0) {
                        externalTransferList.errors.reject('default.internal.server.error', [ex?.cause?.localizedMessage?.substring(0, ex?.cause?.localizedMessage?.indexOf("Detail:"))] as Object[], "")
                    }
                }
                return externalTransferList
    ...
    /**
     * receive externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList receiveData(GrailsParameterMap params) {

        CorrespondenceListStatus correspondenceListStatus
        ExternalTransferList externalTransferList = null
        if (params.encodedId) {

            /**
             * get externalTransfer list
             */
            externalTransferList = ExternalTransferList.get(HashHelper.decode(params.encodedId))

            if (ExternalTransferList) {
                /**
                 * change externalTransfer list status to RECEIVED
                 */
                 ...
                 */
                externalTransferList.manualIncomeNo = params.manualIncomeNo
    ...
    /**
     * to change the externalTransfer List employee status to EMPLOYED in the receive externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList approveExternalTransferRequest(GrailsParameterMap params) {
        /**
         * to get list of  ids of externalTransfer request
         */
        List externalTransferListEmployeeIds = params.listString("check_externalTransferRequestTableInExternalTransferList")
        params.remove("check_externalTransferRequestTableInExternalTransferList")
        ExternalTransferListEmployee externalTransferListEmployee
        List<ExternalTransferListEmployee> externalTransferListEmployeeList
        ExternalTransferList externalTransferList = null

        /**
         * get selected externalTransfer list employee
         */
        GrailsParameterMap externalTransferListEmployeeParam = new GrailsParameterMap(["ids[]": externalTransferListEmployeeIds], WebUtils.retrieveGrailsWebRequest().getCurrentRequest())
        externalTransferListEmployeeList = externalTransferListEmployeeService?.search(externalTransferListEmployeeParam)


        if (externalTransferListEmployeeIds) {

            /**
             * get externalTransferList
             */
            externalTransferList = ExternalTransferList.get(params["id"])


            externalTransferListEmployeeIds?.each { String id ->
                /**
                 * get externalTransfer list employee by id
                 */
                externalTransferListEmployee = externalTransferListEmployeeList?.find { it?.id == id }
                externalTransferList = externalTransferListEmployee?.externalTransferList

                /**
                 * change request and externalTransfer list employee effectiveDate
                 */
                externalTransferListEmployee = saveApprovalInfo(externalTransferListEmployee, params)

                /**
                 * change externalTransfer list employee status
                 */
                externalTransferListEmployee?.recordStatus = EnumListRecordStatus.APPROVED
                /**
                 * note is required in reject.
                 */
                if (params.note || params.orderNo) {
                    externalTransferListEmployee?.addToExternalTransferListEmployeeNotes(new ExternalTransferListEmployeeNote(externalTransferListEmployee: externalTransferListEmployee, orderNo: params.orderNo, note: params.note, noteDate: PCPUtils.parseZonedDateTime(params['noteDate'])))
                }
            }
            try {
                externalTransferList.save(failOnError: true)
     ...
     * @return
     */
    public ExternalTransferListEmployee saveApprovalInfo(ExternalTransferListEmployee record, GrailsParameterMap params){
    ...
    /**
     * to change the externalTransfer List employee status to NOT_EMPLOYED in the receive externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList changeExternalTransferRequestToRejected(GrailsParameterMap params) {
            ...
            if (params.note || params.orderNo) {
                ...
                if (externalTransferListEmployeeIds) {
                    externalTransferListEmployeeIds?.each { String id ->
                        ...
                        /**
                         * get externalTransfer list employee
                         */
                        externalTransferListEmployee = externalTransferListEmployeeList?.find { it?.id == id }
                        externalTransferListEmployee?.effectiveDate = PCPUtils.DEFAULT_ZONED_DATE_TIME
                        /**
                         * change externalTransfer list employee status
                         */
                        externalTransferListEmployee?.recordStatus = EnumListRecordStatus.REJECTED
                    ...
                    try {
                        externalTransferList.save(failOnSave: true)
                        return externalTransferList
                    } catch (Exception ex) {
                        transactionStatus.setRollbackOnly()
                        if (externalTransferList?.hasErrors()) {
                ...
                transactionStatus.setRollbackOnly()
                externalTransferList.errors.reject('externalTransferList.not.requestRejected.message', [] as Object[], "")

            }
        } else {
            transactionStatus.setRollbackOnly()
            externalTransferList.errors.reject('externalTransferList.error.not.selected.request.message', [] as Object[], "")
    ...
    /**
     * close externalTransfer list
     * @param GrailsParameterMap params
     * @return ExternalTransferList
     */
    ExternalTransferList closeList(GrailsParameterMap params) {
        ExternalTransferList externalTransferList = null
        List<ExternalTransferListEmployee> externalTransferListEmployeeList
        if (params.encodedId) {
            ...
            externalTransferListEmployeeList = ExternalTransferListEmployee.executeQuery("From ExternalTransferListEmployee externalTransferListEmployee where externalTransferListEmployee.externalTransferList.id= :externalTransferListId and  externalTransferListEmployee.recordStatus= :recordStatus", [externalTransferListId: externalTransferList?.id, recordStatus: EnumListRecordStatus.NEW])
            //to change the correspondenceListStatus to submitted when we close the externalTransfer list
            CorrespondenceListStatus correspondenceListStatus = new CorrespondenceListStatus()
            correspondenceListStatus.fromDate = PCPUtils.parseZonedDateTime(params['fromDate'])
            correspondenceListStatus.correspondenceList = externalTransferList
            correspondenceListStatus.receivingParty = params["receivingParty"] ? ps.gov.epsilon.hr.enums.v1.EnumReceivingParty.valueOf(params["receivingParty"]) : ps.gov.epsilon.hr.enums.v1.EnumReceivingParty.SARAYA
            ...
            if (ExternalTransferList) {
                ...
                if (correspondenceListStatus.save()) {
                    ...
                    /**
                     * change request status before close the list
                     */
                    ExternalTransferRequest externalTransferRequest = null
                    ...
                    externalTransferList?.externalTransferListEmployees?.each { ExternalTransferListEmployee externalTransferListEmployee ->
                        ...
                        externalTransferRequest = externalTransferListEmployee.externalTransferRequest
                        if (externalTransferListEmployee?.recordStatus == EnumListRecordStatus.APPROVED) {
                            externalTransferListEmployee?.externalTransferRequest?.requestStatus = EnumRequestStatus.APPROVED
                            externalTransferListEmployee?.externalTransferRequest?.effectiveDate = externalTransferListEmployee?.effectiveDate
                        } else {
                            externalTransferListEmployee?.externalTransferRequest?.requestStatus = EnumRequestStatus.REJECTED
                        }

                        // set external order numbers
                        List <ListNote> orderNumberNoteList = externalTransferListEmployee?.externalTransferListEmployeeNotes?.findAll {it.orderNo != null}?.sort{it?.id}
                        if(orderNumberNoteList?.size() > 0){
                            ListNote orderNumberNote = orderNumberNoteList?.last()
                            externalTransferRequest.externalOrderNumber= orderNumberNote.orderNo
                            externalTransferRequest.externalOrderDate= orderNumberNote.noteDate
                        }
                        ...
                        userTermValueList.add("${externalTransferRequest?.employee?.personId}")

                        //we add link into notification to manageExternalTransferList.
                        notificationActionsMap = [
                                [action            : "manageExternalTransferList",
                                 controller        : "externalTransferList",
                                 label             : "${messageSource.getMessage("default.show.label", [] as Object[], LocaleContextHolder.getLocale())}",
                                 icon              : "icon-cog",

                                 notificationParams: [
                                         new NotificationParams(name: "encodedId", value: externalTransferList?.encodedId),
                                 ]
                                ]
                        ]

                        //create notification
                        requestService?.createRequestNotification(externalTransferRequest?.id,
                                ExternalTransferRequest.getName(),
                                ZonedDateTime.now()?.minusDays(1),
                                externalTransferRequest.requestStatus,
                                userTermKeyList,
                                userTermValueList,
                                null,
