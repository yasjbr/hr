...
@Rollback
/**
 * integration test for RecruitmentList service
 */
class RecruitmentListIntegrationSpec extends CommonIntegrationSpec {
    def setupSpec(){
        domain_class =  RecruitmentList
        service_domain =  RecruitmentListService
        entity_name = "recruitmentList"
        required_properties = PCPUtils.getRequiredFields( RecruitmentList)
        hashing_entity = "id"
...
//        }
    ...
    def "test success addApplicants"() {
        ...
        ApplicantStatusHistory applicantStatusHistory = ApplicantStatusHistory.build(applicantStatus: EnumApplicantStatus.TRAINING_PASSED, fromDate: ZonedDateTime.now(), toDate: ZonedDateTime.now().plusYears(1))
        ...
        params["recruitmentListId"] = instanceToSave?.id
        params["id"] = instanceToSave?.id

        when:
        domain_class.withTransaction { status ->
            recruitmentList = serviceInstance.addApplicants(params)
            status.setRollbackOnly()
        }
        then:
        !recruitmentList.hasErrors()
    ...
    def "test fail addApplicants"() {
        ...
        then:
        recruitmentList.hasErrors()
        println("test instance addApplicants fail with result ${recruitmentList}")
    }
     ...
     * @expectedResult valid instance.
     */
    def "test success changeApplicantToEmployed"() {
        setup:
        println("*****************************test success changeApplicantToEmployed******************************************")
        def instanceToSave = saveEntity()
        ApplicantStatusHistory applicantStatusHistory = ApplicantStatusHistory.build(applicantStatus: EnumApplicantStatus.TRAINING_PASSED, fromDate: ZonedDateTime.now(), toDate: ZonedDateTime.now().plusYears(1))
        Applicant applicant = Applicant.build(applicantCurrentStatus:applicantStatusHistory)

        RecruitmentListEmployee recruitmentListEmployee = RecruitmentListEmployee.build(
                applicant: applicant,
                recordStatus:EnumListRecordStatus.NEW,
                recruitmentList:instanceToSave
        )

        println "recruitmentListEmployee: ${recruitmentListEmployee}"
        GrailsParameterMap params = new GrailsParameterMap([:], new MockHttpServletRequest())
        params["check_applicantTableInRecruitmentList"] = [applicant?.id]
        params["recruitmentList.id"] = instanceToSave?.id
        params["id"] = instanceToSave?.id
        ...
        when:
        domain_class.withTransaction { status ->
            map = serviceInstance.changeApplicantToEmployed(params)
            status.setRollbackOnly()
        }
        then:
        println "map: ${map}"
        map.saved == true
        map.errors.size() == 0
        println("test instance changeApplicantToEmployed success is done with errors${map.errors}")
    }

    /**
     * @goal test changeApplicantToEmployed method.
     * @expectedResult null instance and contains errors.
     */
    def "test fail changeApplicantToEmployed"() {
        setup:
        println("*****************************test fail changeApplicantToEmployed******************************************")
        def map
        when:
        GrailsParameterMap params = new GrailsParameterMap([:], new MockHttpServletRequest())
        map = serviceInstance.changeApplicantToEmployed(params)
        ...
        map.saved == false
        map.errors.size() > 0
        println("test instance changeApplicantToEmployed fail with errors ${map.errors}")
    }
    ...
    def "test fail changeApplicantToRejected with no note"() {
        ...
        GrailsParameterMap params = new GrailsParameterMap([:], new MockHttpServletRequest())
        params["check_applicantTableInRecruitmentList"] = [recruitmentListEmployee?.id]
        params["recruitmentList.id"] = instanceToSave?.id
        params["id"] = instanceToSave?.id
