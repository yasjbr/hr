...
@Build([SuspensionExtensionList,ps.gov.epsilon.hr.firm.correspondenceList.CorrespondenceListStatus,ps.gov.epsilon.hr.firm.Firm])
@Domain([SuspensionExtensionList])
@TestMixin([HibernateTestMixin])
class SuspensionExtensionListDomainSpec extends ConstraintUnitSpec {

    void "test SuspensionExtensionList all constraints"() {
        when:
        List<Map> constraints = [
               ...
               // [field: "name", value: new nl.flotsam.xeger.Xeger("(\s?[a-zA-Z\p{InArabic}0-9\.\-\/\_\(\)\*]){2,250}").generate(), testResult: TestResult.PASS],
                ...
                [field: "suspensionExtinsionListEmployees", value: null, testResult: TestResult.PASS],
                [field: "trackingInfo", value: null, testResult: TestResult.PASS],
        ]
        then:
        validateObject(SuspensionExtensionList,constraints)
    }
}
