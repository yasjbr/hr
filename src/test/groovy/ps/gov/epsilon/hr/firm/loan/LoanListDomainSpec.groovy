...
@Build([LoanList,ps.gov.epsilon.hr.firm.correspondenceList.CorrespondenceListStatus,ps.gov.epsilon.hr.firm.Firm])
@Domain([LoanList])
@TestMixin([HibernateTestMixin])
class LoanListDomainSpec extends ConstraintUnitSpec {

    void "test LoanList all constraints"() {
        when:
...
//                [field: "code", value: new nl.flotsam.xeger.Xeger("(\s?[a-zA-Z\p{InArabic}0-9\.]){2,250}").generate(), testResult: TestResult.PASS],
                ...
                [field: "loanListPerson", value: null, testResult: TestResult.PASS],
...
//                [field: "orderNo", value: new nl.flotsam.xeger.Xeger("(\s?[a-zA-Z\p{InArabic}0-9\.]){2,250}").generate(), testResult: TestResult.PASS],
        ...
        then:
        validateObject(LoanList,constraints)
    }
}
