...
import ps.gov.epsilon.hr.firm.recruitment.RecruitmentList
...
/**
 * unit test for RecruitmentList controller
 */
@TestMixin([HibernateTestMixin, GroovyPageUnitTestMixin, ServiceUnitTestMixin])
@Mock([AlertTagLib])
@Domain([RecruitmentList])
@Build([RecruitmentList, Applicant, JoinedFirmOperationDocument, RecruitmentListEmployee, CorrespondenceListStatus])
@TestFor(RecruitmentListController)
class RecruitmentListControllerSpec extends CommonUnitSpec {
    ...
    def setupSpec() {
        domain_class = RecruitmentList
        service_domain = RecruitmentListService
        entity_name = "recruitmentList"
        List required = PCPUtils.getRequiredFields(RecruitmentList)
        required << "code"
        ...
        grailsApplication = Mock(GrailsApplication) {
            getArtefact(_,_) >> new DefaultGrailsClass(RecruitmentList?.class)
        }
    ...
    def "test_list"() {
        ...
        model.operationType == EnumOperation.RECRUITMENT_LIST
        model.referenceObject == RecruitmentList.name
    ...
    def "test_edit"() {
        setup:
        println("************************test_edit********************************")
        def instance1 = saveEntity()
        session_parameters.each { key, value ->
            PCPSessionUtils.setValue(key, getPropertyValue(value, instance1))
        }
        when:
        ...
        instance1?.currentStatus = correspondenceListStatus

        primary_key_values.each { key ->
            controller.params["${key}"] = getPropertyValue(key, instance1, true)
        }
    ...
    /**
     * @goal test manageRecruitmentList action.
     * @expectedResult response with known model.
     */
    def "test_manageRecruitmentList"() {
        setup:
        println("************************test_manageRecruitmentList********************************")
        ...
        controller.manageRecruitmentList()
        then:
        model."${entity_name}" != null
        required_properties.each { String property ->
            getPropertyValue(property, model?."${entity_name}") == entity_name + "_" + property + "_" + counter
        }
        println("test_manageRecruitmentList done with ${entity_name} : ${model."${entity_name}"}")
        counter++
    }
    ...
    @Override
    public RecruitmentList fillEntity(TestDataObject tableData = null) {
        if (!tableData) {
        ...
        if (tableData?.disableSave) {
            ...
            //add details
            def recruitmentListEmployee1 = RecruitmentListEmployee.buildWithoutSave()
            def recruitmentListEmployee2 = RecruitmentListEmployee.buildWithoutSave()
            def recruitmentListEmployee3 = RecruitmentListEmployee.buildWithoutSave()

            instance.addToRecruitmentListEmployees(recruitmentListEmployee1)
            instance.addToRecruitmentListEmployees(recruitmentListEmployee2)
            instance.addToRecruitmentListEmployees(recruitmentListEmployee3)
    ...
    @Override
    public RecruitmentList saveEntity(TestDataObject tableData = null) {
        RecruitmentList instance = fillEntity(tableData)
