...
import ps.gov.epsilon.hr.firm.recruitment.TraineeList
...
/**
 * unit test for TraineeList controller
 */
@TestMixin([HibernateTestMixin, GroovyPageUnitTestMixin, ServiceUnitTestMixin])
@Mock([AlertTagLib])
@Domain([TraineeList])
@Build([TraineeList, Applicant, JoinedFirmOperationDocument, TraineeListEmployee, CorrespondenceListStatus])
@TestFor(TraineeListController)
class TraineeListControllerSpec extends CommonUnitSpec {
    ...
    def setupSpec() {
        ...
        List required = PCPUtils.getRequiredFields(TraineeList)
        required << "code"
        ...
        grailsApplication = Mock(GrailsApplication) {
            getArtefact(_,_) >> new DefaultGrailsClass(TraineeList?.class)
        }
    ...
    def "test_list"() {
        ...
        model.operationType == EnumOperation.TRAINEE_LIST
        model.referenceObject == TraineeList.name
    ...
    /**
     * @goal test manageTraineeList action.
     * @expectedResult response with known model.
     */
    def "test_manageTraineeList"() {
        setup:
        println("************************test_manageTraineeList********************************")
        def instance = saveEntity()
        session_parameters.each { key, value ->
            PCPSessionUtils.setValue(key, getPropertyValue(value, instance))
        }
        ...
        CorrespondenceListStatus correspondenceListStatus = CorrespondenceListStatus.build(fromDate: ZonedDateTime.now(), toDate: PCPUtils.DEFAULT_ZONED_DATE_TIME, correspondenceListStatus: EnumCorrespondenceListStatus.CREATED, receivingParty: EnumReceivingParty.SARAYA)
        instance?.currentStatus = correspondenceListStatus
        controller.manageTraineeList()
        then:
        model."${entity_name}" != null
        required_properties.each { String property ->
            getPropertyValue(property, model?."${entity_name}") == entity_name + "_" + property + "_" + counter
        }
        println("test_manageTraineeList done with ${entity_name} : ${model."${entity_name}"}")
        counter++
    }
    ...
    @Override
    public TraineeList fillEntity(TestDataObject tableData = null) {
        if (!tableData) {
        ...
        if (tableData?.disableSave) {
            ...
            //add details
            def traineeListEmployee1 = TraineeListEmployee.buildWithoutSave()
            def traineeListEmployee2 = TraineeListEmployee.buildWithoutSave()
            def traineeListEmployee3 = TraineeListEmployee.buildWithoutSave()

            instance.addToTraineeListEmployees(traineeListEmployee1)
            instance.addToTraineeListEmployees(traineeListEmployee2)
            instance.addToTraineeListEmployees(traineeListEmployee3)
    ...
    @Override
    public TraineeList saveEntity(TestDataObject tableData = null) {
        TraineeList instance = fillEntity(tableData)
