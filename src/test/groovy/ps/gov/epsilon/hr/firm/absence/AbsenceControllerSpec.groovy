...
/**
 * unit test for Absence controller
 */
@TestMixin([HibernateTestMixin, GroovyPageUnitTestMixin, ServiceUnitTestMixin])
@Mock([AlertTagLib])
@Domain([Absence])
@Build([Absence, Employee, DisciplinaryRequest, JoinedFirmOperationDocument,EmploymentRecord,EmployeePromotion])
@TestFor(AbsenceController)
class AbsenceControllerSpec extends CommonUnitSpec {
    ...
    def setupSpec() {
        domain_class = Absence
        service_domain = AbsenceService
        entity_name = "absence"
        required_properties = PCPUtils.getRequiredFields(Absence)
    ...
    def "test_list"() {
        ...
        model.referenceObject == Absence.name
     ...
     * @expectedResult response with known model.
     */
    def "test_previousAbsenceModal"() {
        setup:
        println("************************test_previousAbsenceModal********************************")
        when:
        controller.params["id"] = 100L
        controller.previousAbsenceModal()
        then:
        model != null
        model != [:]
        model == [employeeId:100L]
        println("test previousAbsenceModal done with model: ${model}")
    }


    /**
     * @goal test createNewAbsence action.
     * @expectedResult response with known model.
     */
    def "test_createNewAbsence"() {
        setup:
        println("************************test_createNewAbsence********************************")
        Absence absence = Absence.build()
        when:
        controller.params["employeeId"] = absence?.employee?.id
        controller.createNewAbsence()
        then:
        model != null
        model != [:]
        println(model)
        model.absence.employee.id == absence?.employee?.id
        println("test createNewAbsence done with model: ${model}")
    }
     ...
     * @expectedResult response with known model.
     */
    def "test_createNewAbsence_not_found"() {
        setup:
        ...
        println("test createNewAbsence done with response: ${response.status}")
    }
