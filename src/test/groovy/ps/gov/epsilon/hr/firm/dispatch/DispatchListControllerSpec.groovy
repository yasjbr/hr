...
/**
 * unit test for DispatchList controller
 */
@TestMixin([HibernateTestMixin, GroovyPageUnitTestMixin, ServiceUnitTestMixin])
@Mock([AlertTagLib])
@Domain([DispatchList])
@Build([DispatchList, DispatchRequest, JoinedFirmOperationDocument, DispatchListEmployee, CorrespondenceListStatus])
@TestFor(DispatchListController)
class DispatchListControllerSpec extends CommonUnitSpec {
    ...
    def setupSpec() {
        ...
        List required = PCPUtils.getRequiredFields(DispatchList)
        required << "code"
    ...
    def "test_list"() {
        ...
        model.operationType == EnumOperation.DISPATCH_LIST
        model.referenceObject == DispatchList.name
    ...
    /**
     * @goal test manageDispatchList action.
     * @expectedResult response with known model.
     */
    def "test_manageDispatchList"() {
        setup:
        println("************************test_manageDispatchList********************************")
        def instance = saveEntity()
        session_parameters.each { key, value ->
            PCPSessionUtils.setValue(key, getPropertyValue(value, instance))
        }
        ...
        CorrespondenceListStatus correspondenceListStatus = CorrespondenceListStatus.build(fromDate: ZonedDateTime.now(), toDate: PCPUtils.DEFAULT_ZONED_DATE_TIME, correspondenceListStatus: EnumCorrespondenceListStatus.CREATED, receivingParty: EnumReceivingParty.SARAYA)
        instance?.currentStatus = correspondenceListStatus
        controller.manageDispatchList()
        then:
        model."${entity_name}" != null
        required_properties.each { String property ->
            getPropertyValue(property, model?."${entity_name}") == entity_name + "_" + property + "_" + counter
        }
        println("test_manageDispatchList done with ${entity_name} : ${model."${entity_name}"}")
        counter++
    }
    ...
    @Override
    public DispatchList fillEntity(TestDataObject tableData = null) {
        if (!tableData) {
        ...
        if (tableData?.disableSave) {
            ...
            //add details
            def dispatchListEmployee1 = DispatchListEmployee.buildWithoutSave()
            def dispatchListEmployee2 = DispatchListEmployee.buildWithoutSave()
            def dispatchListEmployee3 = DispatchListEmployee.buildWithoutSave()

            instance.addToDispatchListEmployees(dispatchListEmployee1)
            instance.addToDispatchListEmployees(dispatchListEmployee2)
            instance.addToDispatchListEmployees(dispatchListEmployee3)
    ...
    @Override
    public DispatchList saveEntity(TestDataObject tableData = null) {
        DispatchList instance = fillEntity(tableData)
